...
// .wrangler/tmp/bundle-xPPZKu/strip-cf-connecting-ip-header.js
...
var isValidServiceAccount = false;
function validateServiceAccountStructure(serviceAccount) {
  ...
  const hasAllFields = requiredFields.every(
    (field) => serviceAccount.hasOwnProperty(field) && serviceAccount[field] !== null && serviceAccount[field] !== void 0 && serviceAccount[field] !== ""
  );
  const isValidType = serviceAccount.type === "service_account";
  const hasValidEmail = serviceAccount.client_email && serviceAccount.client_email.endsWith(".gserviceaccount.com");
  const hasValidPrivateKey = serviceAccount.private_key && serviceAccount.private_key.includes("BEGIN PRIVATE KEY") && serviceAccount.private_key.includes("END PRIVATE KEY");
  return {
...
__name(validateServiceAccountStructure, "validateServiceAccountStructure");
async function validateServiceAccountComplete(serviceAccount) {
  ...
  try {
    ...
    validationResult.checks.iam = true;
    ...
    validationResult.accountInfo = {
      ...
      permissions: [],
      // Permissions will be empty as IAM check is skipped
      keyInfo: keyCheck.keyInfo,
      projectInfo: firebaseCheck.projectInfo
    };
    validationResult.valid = validationResult.checks.authentication && validationResult.checks.iam && validationResult.checks.firebase;
...
__name(validateServiceAccountComplete, "validateServiceAccountComplete");
...
__name(validateFirebaseProject, "validateFirebaseProject");
...
__name(validateFirestoreAccess, "validateFirestoreAccess");
async function validateServiceAccountKey(serviceAccount, accessToken) {
  try {
    ...
    const currentKey = data.keys?.find(
      (key) => key.name.includes(serviceAccount.private_key_id)
    );
    if (!currentKey) {
    ...
    return {
      ...
      keyInfo: {
        ...
        daysUntilExpiry: Math.ceil((validBefore - now) / (1e3 * 60 * 60 * 24))
      }
    };
  } catch (error) {
    return {
...
__name(getRootCollectionIds, "getRootCollectionIds");
async function getDocumentsInCollection(projectId, accessToken, collectionName) {
...
__name(getDocumentsInCollection, "getDocumentsInCollection");
...
async function uploadDocument(projectId, accessToken, collectionName, docId, docData) {
...
__name(convertToFirestoreFields, "convertToFirestoreFields");
...
__name(parseMultipartForm, "parseMultipartForm");
      ...
      return c.json(
        {
          ...
          details: validationResult.errors.map((e) => `${e.type}: ${e.message}`).join("; ")
        },
        400
      );
    }
...
app.post("/upload-collection", async (c) => {
  if (!isValidServiceAccount) {
    return c.json(
      {
        success: false,
        error: "Valid service account required. Please validate your service account first."
      },
      401
    );
  }
...
// .wrangler/tmp/bundle-xPPZKu/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-xPPZKu/middleware-loader.entry.ts
